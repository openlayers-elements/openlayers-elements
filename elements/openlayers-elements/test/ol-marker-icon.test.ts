...
describe("ol-marker-icon", () => {
  it("defaults to [0,0] position", async () => {
    ...
    // then
    expect(feature.createFeature().getGeometry().getCoordinates()).to.deep.equal([0, 0]);
  })
  ...
  it("passes coordinates from x/y properties", async () => {
    ...
    // then
    expect(feature.createFeature().getGeometry().getCoordinates()).to.deep.equal([16, 24]);
  });
  ...
  it("passes coordinates from lon/lat properties", async () => {
    ...
    // then
    expect(feature.createFeature().getGeometry().getCoordinates()).to.deep.equal([-410026.7539820607, 4924240.587205196]);
  });
});
