...
async function fixture(template): Promise<any> {
    const layer = await mapFixture<OlLayerVector>(
        html`<ol-layer-vector>${template}</ol-layer-vector>`,
        'ol-layer-vector')
...
describe('ol-marker-icon', () => {
    it('defaults to [0,0] position', async () => {
        // given
        const feature = await fixture(html`<ol-marker-icon src="${dotUrl}"></ol-marker-icon>`)
    ...
    it('passes coordinates from x/y properties', async () => {
        // given
        const feature = await fixture(html`<ol-marker-icon src="${dotUrl}"
                                            x="16" y="24"></ol-marker-icon>`)
